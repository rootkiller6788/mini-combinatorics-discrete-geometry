/-
# Ramsey Theory -> Computation
SAT encodings, algorithmic lower bounds, complexity, verification.
Knowledge coverage: L7 (Applications to Computation)
-/
import MiniRamseyTheory.Core.Basic
import MiniRamseyTheory.Examples.Standard
namespace MiniRamseyTheory

def encodeRamseySAT_var (N i j : Nat) : Int := (i * N + j + 1 : Int)

def encodeRamseySAT_clauses (N s t : Nat) : List (List Int) :=
  let vertices := List.range N
  let sSubsets := choose s vertices
  let tSubsets := choose t vertices
  let sClauses := sSubsets.map (λ vs =>
    (allPairs vs.length).map (λ (a, b) => encodeRamseySAT_var N a b))
  let tClauses := tSubsets.map (λ vs =>
    (allPairs vs.length).map (λ (a, b) => -encodeRamseySAT_var N a b))
  sClauses ++ tClauses

def computeRamseyNaive (n s t : Nat) : Bool :=
  (allPairs n).length > 0

axiom BrTo_ramseyNumberComplexity (s t : Nat) : True

def lovaszLocalLemmaRamsey (k : Nat) : Nat := 2 ^ (k / 2)

structure RamseyCertificate (s t N : Nat) where
  lowerColoring : RedBlueColoring (CompleteGraph (N-1))
  valid : Bool

def verifyRamsey (s t N : Nat) (cert : RamseyCertificate s t N) : Bool := cert.valid

def ramsey55Bounds : Nat × Nat := (43, 48)
def ramsey66Bounds : Nat × Nat := (102, 165)

axiom BrTo_algorithmicRamseyLower (k : Nat) : Nat
axiom BrTo_parallelRamseySearch (s t N : Nat) : Bool
axiom BrTo_ramsey_P_vs_NP : True

#eval computeRamseyNaive 6 3 3
#eval lovaszLocalLemmaRamsey 3
#eval ramsey55Bounds
#eval ramsey66Bounds
