/-
# Ramsey Theory -> Algebra
Knowledge coverage: L7
-/
import MiniRamseyTheory.Core.Basic
import MiniRamseyTheory.Theorems.Basic
namespace MiniRamseyTheory

axiom BrTo_RamseyGroup (G : Type) : Type
axiom BrTo_additiveGroupRamsey : True

def isSumFree (A : List Nat) : Prop :=
  ∀ a ∈ A, ∀ b ∈ A, a + b ∉ A

axiom BrTo_largestSumFreeSize (n : Nat) : ∃ A, isSumFree A ∧ A.length = n/2

axiom BrTo_legendreSymbol (a p : Nat) : Fin 2
axiom BrTo_quadraticReciprocity_ramsey (p q : Nat) : True

axiom BrTo_schur_sumfree_connection : schurNumber 2 = 5
